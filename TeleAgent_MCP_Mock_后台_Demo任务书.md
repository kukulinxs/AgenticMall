...
后续可在 `admin-web/` 增加一个独立的商品运营前端，用于和 TeleAgent 演示同一份商品数据。该前端仅复用 Vue 3、Vite 和 Element Plus 的基础能力，直接调用 `/api/shop/goods/*`；不得为 Demo 引入完整商城、订单、会员、促销、登录、Redis、数据库迁移或复杂权限框架。

首个后台版本的范围固定为：商品列表、名称/部门/状态筛选、新增、编辑、上下架、删除，以及基本加载/错误提示。它与 MCP 网关共享商品业务函数与内存数据，前端不另行维护 Mock 数据或第二套业务规则。
